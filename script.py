...
class LandRegulationsRAG:
    ...
    def get_embedding(self, text: str) -> list:
        """调用大模型获取用户提问的向量 (自带重试与防崩机制)"""
        import dashscope
        import time
        # 不要设置 dashscope.base_url
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    if embeddings and len(embeddings) > 0:
                        ...
                        print(f"  ⚠️ [重试 {attempt+1}/3] 阿里云 API 返回为空，稍后重试...")
                else:
                    print(f"  ⚠️ [重试 {attempt+1}/3] API 报错: {resp.message}")
            except Exception as e:
                print(f"  ⚠️ [重试 {attempt+1}/3] 网络请求异常: {e}")
            
            time.sleep(1.5) # 失败后停顿 1.5 秒再重试
            
        print("  ❌ 提取提问向量彻底失败，本次查询将降级为纯图谱检索。")
        return []

    def vector_search(self, user_question: str, top_k: int = 3) -> list:
        """执行纯向量语义检索"""
        query_vector = self.get_embedding(user_question)
        if not query_vector:
            return [] # 如果没拿到向量，直接安全退出
            
        cypher_query = """
        CALL db.index.vector.queryNodes('article_text_index', $top_k, $query_vector)
        YIELD node, score
        // WHERE score > 0.85  // 暂时关闭阈值，保证最大召回率
        RETURN labels(node) AS labels, coalesce(node.number, node.name) AS name, 
               coalesce(node.description, '') AS description, 
               coalesce(node.full_text, '') AS full_text, score
        """
        try:
            with self.driver.session() as session:
            ...
            print(f"⚠️ 向量检索执行出错: {e}")
            return []

    def execute_hybrid_search(self, question: str, primary_cypher: str):
        """4.3.2 图结构检索 + 向量语义检索的并行协同与融合"""
        graph_results = []
        vector_results = []
        strategy = "纯向量检索兜底"

        # 1. 尝试执行精准图检索 (Text2Cypher)
        if primary_cypher:
            graph_results = self.execute_cypher(primary_cypher)

        # 2. 并行执行向量语义检索 (补充语义上下文)
        ...
        def extract_text(item):
            ...
            if "node" in item and isinstance(item["node"], dict):
                return item["node"].get("full_text", item["node"].get("description", ""))
            return item.get("full_text", item.get("description", ""))

        # 优先合并高精度的图检索结果
        for item in graph_results:
            text = str(extract_text(item))
            if text and text[:50] not in seen_texts: # 用前50个字作为文本指纹
        ...
        if graph_results and vector_results:
            strategy = "图谱精准查询 + 向量语义补充 (双引擎联合)"
        elif graph_results and not vector_results:
            strategy = "纯图谱精准查询"

        return merged_results, primary_cypher, strategy
