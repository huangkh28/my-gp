...
def run_eval():
    ...
    for i, item in enumerate(test_data):
        ...
        if not cypher_query:
            print("  ⚠️ [生成超时/失败] 准备触发向量语义双路协同...")

        # 2. 执行图检索与向量检索协同
        try:
            # 注意：按照我们在 script.py 中新写的 execute_hybrid_search，它返回三个值
            db_results, actual_cypher, retrieval_strategy = rag.execute_hybrid_search(
                question, cypher_query
            )

            # 👇 重新定义“彻底失败”：图检索和向量检索都什么都没捞到
            if not db_results:
                print("  ❌ [彻底失败] 图检索与向量检索均未召回任何数据")
                print("-" * 60)
                ...
                with open(log_file, "a", encoding="utf-8") as f:
                    ...
                    f.write(
                        f"【失败原因】: Cypher生成失败，且向量检索也未召回任何数据\n"
                    )
                    f.write("-" * 50 + "\n\n")
                continue  # 彻底失败，直接跳过本题后续验证
            else:
                # 只要不是彻底失败，且 Cypher 不为空，就算一次语法成功
                if cypher_query:
                    cypher_success_count += 1

                print(
                    f"  ✅ [协同召回] 策略: {retrieval_strategy} | 召回数据 {len(db_results)} 条"
                )
